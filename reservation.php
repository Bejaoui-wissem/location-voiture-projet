...
$required = ['voiture_id', 'nom_client', 'telephone', 'date_debut', 'date_fin', 'prix_total', 'mode_paiement', 'agence_retrait', 'date_permis'];
...
// Validation permis 2 ans minimum
$datePermis = new DateTime($data['date_permis']);
$aujourdhui = new DateTime();
$diff = $aujourdhui->diff($datePermis);
$ageMois = ($diff->y * 12) + $diff->m;
if ($ageMois < 24) {
    http_response_code(400);
    echo json_encode([
        'success' => false,
        'message' => "Permis trop recent (" . $ageMois . " mois). Minimum 2 ans requis pour louer un vehicule."
    ]);
    exit;
}
...
// Verifier que la voiture n'est pas en panne/accident/reparation
if (in_array($voiture['statut'], ['accidentee', 'en_panne', 'en_reparation'])) {
    echo json_encode(['success' => false, 'message' => 'Cette voiture n\'est pas disponible (en ' . str_replace('_', ' ', $voiture['statut']) . ')']);
    exit;
}
...
$stmt = $pdo->prepare("
    ...
    (voiture_id, nom_client, telephone, email_client, date_permis, date_debut, date_fin, prix_total, mode_paiement, numero_carte, paiement_valide, agence_retrait, date_limite_retrait)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
");
$stmt->execute([
